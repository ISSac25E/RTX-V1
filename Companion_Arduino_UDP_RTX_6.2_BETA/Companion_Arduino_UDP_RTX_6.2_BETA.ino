...
const uint16_t COMPANION_OSC_LCP = 12321;
...
void loop () {
  ...
  COMPANION_RUN();
  TWOWIRE_EXECUTE();
  if ((millis() - OSC_SEND_TIMER) >= 1000 and !UDP_CONNECTION_SLEEP) {
    {
      OSCMessage Page1Bank2TextOSC("/style/text/1/2");
      uint16_t TEMP_INT_HOLD = (3600000 - (millis() - UDP_CONNECTION_TIMER) / 1000);
      uint16_t TEMP_MINUTE_HOLD = 0;
      while (TEMP_INT_HOLD >= 60) {
        TEMP_INT_HOLD -= 60;
        TEMP_MINUTE_HOLD++;
      }
      {
        String TempStr = String(TEMP_MINUTE_HOLD);
        char TempChar[TempStr.length() + 1];
        memset(TempChar, 0, TempStr.length() + 1);
        TempStr.toCharArray(TempChar, TempStr.length() + 1);
        Page1Bank2TextOSC.add(TempChar);
      }
      Page1Bank2TextOSC.add(":");
      {
        String TempStr = String(TEMP_INT_HOLD);
        char TempChar[TempStr.length() + 1];
        memset(TempChar, 0, TempStr.length() + 1);
        TempStr.toCharArray(TempChar, TempStr.length() + 1);
        Page1Bank2TextOSC.add(TempChar);
      }
      TX_UDP.beginPacket(COMP_IP, COMPANION_OSC_LCP);
      Page1Bank2TextOSC.send(TX_UDP);
      TX_UDP.endPacket();
      Page1Bank2TextOSC.empty();
      OSC_SEND_TIMER = millis();
    }
  }
}
