...
uint32_t Tally_Error_LED_Timer = millis();
...
uint8_t Tally_PWNCounter = 0;
uint8_t Tally_PWMDelay = 8;
...
void RTX_TallyCheck() {
  ...
  if (PacketNumber == TallyPacketCount or PacketNumber == 0) {
    ...
    if (PacketNumber == 2) {
      ...
      if(Tally_Connected) TallyUpdate();
      TallyPacketCount = 0;
    }
  }
  else {
...
void TallyUpdate(){
  if(bitRead(TallyStatus[5], 0)) analogWrite(5, 100);
  else analogWrite(5, 0);
  if(bitRead(TallyStatus[5], 1) and !bitRead(TallyStatus[5], 0)) analogWrite(6, 100);
...
void Tally_Error() {
  if(!Tally_Connected){
    analogWrite(6, 0);
    if ((millis() - Tally_Error_LED_Timer) > Tally_PWMDelay) {
      Tally_Error_LED_Timer = millis();
      Tally_PWMDelay = 8;
      if (Tally_Error_LED_State) {
        if (Tally_PWNCounter > 0)
          Tally_PWNCounter -= 5;
        else {
  ...
  else if((millis() - Tally_Timer) > 500) Tally_Connected = false;
}
