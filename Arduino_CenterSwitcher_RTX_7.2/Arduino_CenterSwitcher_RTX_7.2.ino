...
uint8_t ButtonPIN[] {12, 2, 3, 4, 7};
uint32_t ButtonTimer[5];
bool ButtonTimerState[5];
bool ButtonState[5];
...
uint32_t Tally_LED_Timer_GREEN = millis();
...
bool Tally_LED_State_GREEN;
uint16_t Tally_PWN_Counter = 100;
uint16_t Tally_PWM_Delay = 8;
uint16_t Tally_PWN_Counter_GREEN = 100;
uint16_t Tally_PWM_Delay_GREEN = 8;
...
void LED_Run() {
  if (!EEPROM_ERROR) {
    if (Comp_Connected) {
      ...
      if ((millis() - Tally_LED_Timer_GREEN) > Tally_PWM_Delay_GREEN) {
        Tally_LED_Timer_GREEN = millis();
        if (Tally_LED_State_GREEN) {
          Tally_LED_State_GREEN = false;
          Tally_PWM_Delay_GREEN = 400;
          analogWrite(LED_TallyPIN[1], 0);
        }
        else {
          Tally_LED_State_GREEN = true;
          Tally_PWM_Delay_GREEN = 50;
