...
void CheckSerial_Run() {
  if (IP_Enter) {
    if (Serial.available()) {
      int SerialCheckTemp = SerialParse(0);
      if (SerialParse(0) == -1) {
    ...
    if (Serial.available()) {
      int SerialCheckTemp = SerialParse(1);
      if (bitRead(SerialCheckTemp, 15)) {
        ...
        else if (SerialCheckTemp == 'D') if (SerialParse(1) == -1) RTX_GUI_DATA_DUMP();
          }
      else {
        SerialBuffer[0] = SerialCheckTemp;
        SerialBuffer[1] = SerialParse(1);
        if (SerialBuffer[1] == -1){
          if (SerialBuffer[0] >= 0 and SerialBuffer[0] <= 255) RTX_GUI_READ(SerialBuffer[0]);
        }
            else if (!bitRead(SerialBuffer[1], 15))
              if (SerialParse(1) == -1)
                if (SerialBuffer[0] >= 0 and SerialBuffer[0] <= 255 and SerialBuffer[1] >= 0 and SerialBuffer[1] <= 255) RTX_GUI_WRITE(SerialBuffer[0], SerialBuffer[1]);
                }
      while (Serial.available()) Serial.read();
    }
  }
...
void RTX_GUI_READ(uint8_t REGISTER_ADDRESS) {
  uint8_t RTX_BufferSendTemp[2] = {0, 1};
  for (uint8_t X = 0; X < 4; X++) bitWrite(RTX_BufferSendTemp[0], X + 4, bitRead(REGISTER_ADDRESS, X));
  for (uint8_t X = 0; X < 4; X++) bitWrite(RTX_BufferSendTemp[1], X + 2, bitRead(REGISTER_ADDRESS, X + 4));
        ...
        if (PacketNumber == PacketCount) {
          ...
          if (PacketNumber == 2) {
            ...
            if (!RTX_BufferRecieveTemp[0]) {
              Recieved = true;
              Serial.print(F("READ: "));
              //                      for (int8_t Z = 7; Z > -1; Z--) Serial.print(bitRead(RTX_BufferRecieveTemp[1], Z));
              //                      Serial.print(F("("));
              Serial.print(RTX_BufferRecieveTemp[1]);
              //                      Serial.print(F(")"));
              Serial.print(F(", "));
              for (int8_t Z = 7; Z > -1; Z--) Serial.print(bitRead(RTX_BufferRecieveTemp[2], Z));
              Serial.print(F("("));
              Serial.print(RTX_BufferRecieveTemp[2]);
              Serial.println(F(")"));
              Serial.println();
              Serial.println(F("ENTER COMMAND"));
            }
...
void RTX_GUI_WRITE(uint8_t REGISTER_ADDRESS, uint8_t WRITE_DATA) {
  uint8_t RTX_BufferSendTemp[3] = {4, 1, 2};
  for (uint8_t X = 0; X < 4; X++) bitWrite(RTX_BufferSendTemp[0], X + 4, bitRead(REGISTER_ADDRESS, X));
  for (uint8_t X = 0; X < 4; X++) bitWrite(RTX_BufferSendTemp[1], X + 2, bitRead(REGISTER_ADDRESS, X + 4));
  for (uint8_t X = 0; X < 2; X++) bitWrite(RTX_BufferSendTemp[1], X + 6, bitRead(WRITE_DATA, X));
  for (uint8_t X = 0; X < 6; X++) bitWrite(RTX_BufferSendTemp[2], X + 2, bitRead(WRITE_DATA, X + 2));
  TwoWire_G_BufferTX[0] = SelectedIP;
...
int16_t SerialParse(bool HEX_BIN_ENABLE) {
  delay(5);
  if (Serial.available() > 1) {
    ...
    if (TempSerialHold >= 48 and TempSerialHold <= 57) {
    ...
    else if (HEX_BIN_ENABLE and TempSerialHold == 'B') {
      
      bool TempBinBuffer[8];
      uint8_t BitCount = 0;
       
      while (Serial.available() and TempSerialHold >= 48 and TempSerialHold <= 49) {
        if (BitCount < 8) {
          TempBinBuffer[BitCount] = (TempSerialHold - 48);
          BitCount++;
        }
        TempSerialHold = Serial.read();
      }
      ...
      while (BitCount) {
        BitCount--;
        bitWrite(Result, TempCount, TempBinBuffer[BitCount]);
        TempCount++;
      }
      return Result;
    }
    else if(HEX_BIN_ENABLE and TempSerialHold == 'H') {
      Result = 'H';
      bitWrite(Result, 15, 1);
      
      TempSerialHold = Serial.read();
      uint8_t TempHexBuffer[2];
      uint8_t BitCount = 0;
      
      for(uint8_t X = 0; X < 2 and Serial.available(); X++) {
       TempSerialHold = Serial.read();
       if((TempSerialHold >= 48 and TempSerialHold <= 49){
        BitCount++;
        TempHexBuffer[BitCount] = TempSerialHold;
       } else switch(TempSerialHold) {
        case 'a':
        break;
        case 'b':
        break;
