...
void loop() {
  ...
  if ((millis() - ATEM_ConnectionTimer) > 1000) {
...
void COMPANION_RUN() {
  ...
  if (!UDP_CONNECTED and (millis() - DIS_TIMER) >= 5000) {
    //pinMode(4, OUTPUT);
    TX_UDP.stop();
    RX_UDP.stop();
    TX_UDP.begin(UDP_TX_LCP);
...
void CHECK_UDP_PACKETS() {
  ...
  while (RX_UDP.available()) RX_UDP.read(UDP_BUFFER, 10);
}
...
void RTX_TallySend() {
  ...
  bitWrite(TwoWire_G_BufferTX[1], 6, UDP_CONNECTED);
...
void ATEM_RUN() {
  ...
  if (UDP.available() && ATEM_T_PacketSize != 0)   {
    ...
    ATEM_V_PacketID = word(ATEM_V_PacketBuffer[10], ATEM_V_PacketBuffer[11]);
    Serial.print("PACKET_ID: ");
    Serial.println(ATEM_V_PacketID);
    ...
    if (ATEM_T_PacketSize == ATEM_T_PacketLength) {
      ...
      if (ATEM_T_CommandACK) {
        ...
        if(ATEM_V_PacketID == ATEM_V_LastRemotePacketID + 1) ATEM_V_LastRemotePacketID++;
        Serial.print("ACK: ");
        Serial.println(ATEM_V_LastRemotePacketID);
...
void ATEM_ParsePacket(uint16_t ATEM_T_PacketLength2) {
  ...
  while (ATEM_T_IndexPointer < ATEM_T_PacketLength2 and !ATEM_ParsePacket_Done) {
    ...
    if (ATEM_T_CommandLength > 2 && ATEM_T_CommandLength <= 96) {
      ...
      Serial.print("CMD: ");
      Serial.print(ATEM_T_CommandString);
...
//      }
      if (strcmp( ATEM_T_CommandString, "TlIn") == 0) {
        ...
        Serial.print(": ");
        for(uint8_t X = 0; X < 8; X++) Serial.print(ATEM_V_TallyInput[X]);
      }
