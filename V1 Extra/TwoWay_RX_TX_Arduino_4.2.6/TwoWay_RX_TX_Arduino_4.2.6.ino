...
uint8_t const TwoWire_G_BufferSize = 12;
        ...
        if (TwoWire_Read(HIGH, HIGH)) {
          switch (TwoWire_CheckMessage(I)) {
            ...
            case 1:
              ...
              break;
            case 2:
            TwoWire_BufferStore(I, TwoWire_RX_Buffer[3], TwoWire_RX_Buffer[4]);
              TwoWire_CompileMessage(TwoWire_G_SlaveIP[I], TwoWire_Command(1,!TwoWire_TXStatus[0] ,TwoWire_TXStatus[1] ), 0, 0, LOW);
              if (TwoWire_Safe()) {
                TwoWire_Send();
              }
              TwoWire_CheckConnectionInterval(I, 1);
...
boolean BufferRX(uint8_t TwoWire_SlaveIP) {
  if (TwoWire_CheckConnectionInterval(TwoWire_SlaveIP,0)) {
...
boolean TwoWire_BufferStore(uint8_t TwoWire_SenderIP, uint8_t TwoWire_RecieverIP, uint8_t TwoWire_Data) {  //Check if IP valid and if there is space in buffer for it then stores it in
  bool TwoWire_FoundIP = false;
  TwoWire_G_SlaveIP_Result = 0;
  TwoWire_TXStatus[0] = 0;
  TwoWire_TXStatus[1] = 0;
  for ( uint8_t I = 0; I < TwoWire_G_SlaveIPCount and !TwoWire_FoundIP; I++) {
    if (TwoWire_RecieverIP == TwoWire_G_SlaveIP[I]) {
      TwoWire_FoundIP = true;
      TwoWire_G_SlaveIP_Result = I;
    }
  }
  if (TwoWire_FoundIP and TwoWire_CheckConnectionInterval(TwoWire_G_SlaveIP_Result,0)) {
    TwoWire_TXStatus[0] = true;
