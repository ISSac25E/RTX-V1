...
void loop() {
  TwoWire_Run(0);
  digitalWrite(13,TwoWire_Connected);
}

void TwoWire_Execute(uint8_t TwoWire_T_IP, uint8_t TwoWire_T_Data, bool TwoWire_T_Send){
  if(TwoWire_T_Send){
    TwoWire_TX_Buffer[0] = TwoWire_T_IP;
    TwoWire_TX_Buffer[1] = TwoWire_T_Data;
    bool Done = false;
    for(int I = 0; I < 3 and !Done; I++){
    TwoWire_Run(1);
    if(TwoWire_Status[1])
    Done = true;
