...
void TallyUpdate() {
  if (Tally_Connected) {
    if (TallyStatus[1][0]) {
      ...
      Tally_PWMDelay = 0;
      analogWrite(5, 0);
      if (TallyStatus[1][1]) analogWrite(6, 100);
...
void Button_Execute() {
  ...
  if (digitalRead(12) and ButtonState12) {
    ...
    if (digitalRead(12)) {
      ...
      else if ((millis() - ButtonTimer12) > 20 and !AutoMode) {
        if (!Error) {
          Mode = !Mode;
          if (Mode) {
            analogWrite(11, 0);
