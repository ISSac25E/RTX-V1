#include <EEPROM.h>
const bool ControllerSide = 1; //0 = Center, 1 = Side
...
void RTX_GUI_UPDATE_VALUE() {
  ...
  if (!EEPROM_WRITING){
    EEPROM_WRITING = bitRead(GUI_Registers[251], 1);
    RTX_GUI_UPDATE_REGISTER(5);
  }
  if (EEPROM_ERROR) EEPROM_ERROR = bitRead(GUI_Registers[251], 2);
}
...
void Button_Execute() {
  ...
  if (digitalRead(ButtonPIN[0]) and ButtonState[0]) {
    ...
    if (digitalRead(ButtonPIN[0])) {
      ...
      if ((millis() - ButtonTimer[0]) >= 300) bitWrite(Mode, 1, !bitRead(Mode, 1));
      else bitWrite(Mode, 0, !bitRead(Mode, 0));
